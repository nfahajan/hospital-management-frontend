...
import { Plus, Trash2, Calendar, Clock, Stethoscope } from "lucide-react";
...
import { useUpdateDoctorMutation } from "@/redux/features/doctorApi";
...
export function EditDoctorForm({
  ...
  const onSubmit = async (data: UpdateDoctorFormData) => {
    try {
      await updateDoctor({ id: doctor._id, data }).unwrap();
  ...
  const validateCurrentStep = async (): Promise<boolean> => {
    ...
    // If no fields to validate (like Step 4 with optional bio), always return true
    if (fieldsToValidate.length === 0) {
      console.log("No fields to validate for this step, validation passed");
      return true;
    }
  ...
  const getFieldsForStep = (step: number): string[] => {
    switch (step) {
      ...
      case 3: // Bio & Final Details (optional field, no validation needed)
        return [];
      default:
        return [];
    }
  ...
  const hasStepErrors = (step: number): boolean => {
    ...
    // If no fields to check (like Step 4), no errors
  ...
  const getStepDescription = (step: number) => {
    switch (step) {
      case 1:
        return "Update personal and contact information";
      case 2:
        return "Update professional qualifications, experience, and availability";
      case 3:
        return "Update bio and finalize changes";
      default:
        return "";
    }
  };
          ...
          {currentStep === 4 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Update bio and finalize changes
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <FormField
